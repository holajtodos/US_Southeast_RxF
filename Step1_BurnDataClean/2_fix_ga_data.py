...
def check_conditions(state_abbr_local, contains_agriculture_keyword):
    pattern = _to_regex(contains_agriculture_keyword)
    if pattern:
        contains_agriculture = df_sub['BURN_TYPE'].fillna("").astype(str).str.contains(
...
if state_abbr == 'FL':
    check_conditions('FL', 'Agricultur')
elif state_abbr == 'GA':
    check_conditions('GA', ['CROP', 'ORCHARD', 'PASTURE'])
elif state_abbr == 'MS':
    check_conditions('MS', 'Agricultur')
elif state_abbr == 'TN':
    check_conditions('TN', 'Agricultur')
elif state_abbr == 'SC':
    check_conditions('SC', ['PASTURE', 'DITCH', 'DISEASE'])
else:
    for index, row in df_sub.iterrows():
        fuel_type = row["fuel_type"]
        lat_lon = (row['LATITUDE'], row['LONGITUDE'])
...
res_df.to_csv("SE_Combined_Permit_rx_lf_3states_" + str(selected_year) + ".csv", index=False, header=True)
...
res_df.to_csv("SE_Combined_Permit_agr_lf_3states_" + str(selected_year) + ".csv", index=False, header=True)
...
res_df.to_csv("SE_Combined_Permit_invalid_lf_3states_" + str(selected_year) + ".csv", index=False, header=True)

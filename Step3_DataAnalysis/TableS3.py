...
out_csv = os.path.join(dir_python_local, "Table_S5_E-tot_PM25_metrics.csv")
# -----------------------------------------------
...
for c in col_order:
    if c not in table_s5.columns:
        table_s5[c] = np.nan

table_s5 = table_s5[col_order]

# Round numeric columns nicely (2 decimals for % and r), keep # Pairs as int
pct_cols = [c for c in table_s5.columns if "(%)" in c]
r_cols   = ["r", "r_17", "r_18", "r_19"]

for c in pct_cols + r_cols:
    table_s5[c] = table_s5[c].astype(float).round(2)

table_s5["# Pairs"] = table_s5["# Pairs"].fillna(0).astype(int)
...
table_s5["State"] = table_s5["State"].astype(state_cat)
table_s5["Burn season"] = table_s5["Burn season"].astype(season_cat)
table_s5 = table_s5.sort_values(["State", "Burn season"]).reset_index(drop=True)

# Save to CSV
table_s5.to_csv(out_csv, index=False)
print(f"\nSaved to: {out_csv}")
...
HIGH_FIRE_SITE_DATA = {
    "SiteId": [
        120470015, 120730012, 130210012, 130590002, 130950007, 131510002, 
        131530001, 131850003, 132150008, 450290002, 450370001
    ],
    "Latitude": [
        30.4266, 30.4397, 32.8053, 33.9181, 31.5776, 33.4339, 
        32.6056, 30.8486, 32.5213, 33.0079, 33.7400
    ],
    "Longitude": [
        -82.7947, -84.3464, -83.5435, -83.3444, -84.0998, -84.1618, 
        -83.5978, -83.2933, -84.9446, -80.9650, -81.8536
    ]
}
...
def generate_figure_b(gdf_SE):
    ...
    criteria = lambda row: not (round(row['Spearman R'], 1) >= 0.4)
    ...
    for k, metric in enumerate(selected_metrics):
        ...
        if metric == '# Pairs':
            cbar = plt.colorbar(sc1, ax=ax, orientation='vertical', extend='max', shrink=0.6, pad=0.01)
        elif metric == 'MB':
            cbar = plt.colorbar(sc1, ax=ax, orientation='vertical', extend='both', shrink=0.6, pad=0.01)
        elif metric == 'Spearman R':
            cbar = plt.colorbar(sc1, ax=ax, orientation='vertical', extend='min', shrink=0.6, pad=0.01)
        else:
            cbar = plt.colorbar(sc1, ax=ax, orientation='vertical', shrink=0.4, pad=0.01)
...
def generate_figure_c():
    ...
    sns.histplot(
        data=all_years_subset,
        x="log_PM_TOT_mod_rxf", y="log_smokePM",
        stat='count', cmap=cc.cm.CET_L19,
        cbar=True,
        cbar_ax=cbar_ax,
        cbar_kws=dict(
            shrink=0.85,
            label='Count',
            extend='max'   # ← add this line
        ),
        ax=g0.ax_joint,
        binwidth=0.1
    )
    ...
    # Regression + 1:1 line
    x_vals = np.linspace(-2, np.log10(80), 100)
    ...
    g0.ax_joint.set_xlim(-2, np.log10(80))
    g0.ax_joint.set_ylim(-2, np.log10(80))

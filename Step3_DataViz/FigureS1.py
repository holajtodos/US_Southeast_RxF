...
# csv_tpl = "/home/jh94030/scripts/python/postdoc_project/rxfire/data/SE_permit_data_2010-2020/update_criteria/SE_Combined_Permit_lf_rx_{year}.csv"
...
#     t = ax_main.text(lon, lat, state, color=color, fontweight=weight, fontsize=12,
...
#                handletextpad=0.25, borderaxespad=0.5, alignment="left", fontsize=9, title_fontsize=10)
...
#     color="k", linewidth=1.5, transform=ccrs.PlateCarree()
# )
...
# ----------------------------- Style Constants --------------------------------
year_colors = {2017: "#0D1282", 2018: "#C06014", 2019: "#D71313"}
year_sizes  = {2017: 0.1, 2018: 0.1, 2019: 0.1}
year_zorders = {2017: 1, 2018: 2, 2019: 3}
...
for year in [2017, 2018, 2019]:
    ...
    gdf_SE.boundary.plot(
        ax=ax_main, edgecolor="k", linewidth=0.7,
    ...
    for state, (lon, lat) in state_labels.items():
        ...
        weight = "bold" if state in ["FL", "GA", "SC"] else "semibold"
        t = ax_main.text(
            lon, lat, state, color=color, fontweight=weight, fontsize=12,
            transform=ccrs.Geodetic(), zorder=10
        )
        t.set_path_effects([
            path_effects.Stroke(
    ...
    # legend_elements = [
    #     Line2D(
    #         [0], [0], marker="o", color="w", label=str(year),
    #         markerfacecolor="w", markersize=3,
    #         markeredgecolor=year_colors[year]
    #     )
    # ]
    legend_elements = [
    Line2D([0], [0], marker='o', color='w', label='2017',
    ...
    ax_main.legend(
        handles=legend_elements, title="Rx Fire Records", loc="center right",
        handletextpad=0.25, borderaxespad=0.5, alignment="left",
    ...
    # Optional title
    # ax_main.set_title(f"The Southeast Prescribed (Rx) Fire Permit Database: {year}",
    #                   loc="left", fontsize=10, pad=10)

    # ----------------------------- Save --------------------------------------
    out_png = f"SE_RxF_spatial_map_{year}.png"

###############################################################################
# Grid_permits_to_CMAQ.py
#
# Purpose: Regrid permit-based (BlueSky) point-level Rx fire emissions
#          onto the CMAQ 12US1 Lambert Conformal Conic grid defined by a
#          METCRO2D file.
#
# Method:
#   1. Read CMAQ grid parameters (LCC projection, XORIG, YORIG, XCELL,
#      YCELL, NCOLS, NROWS) from a METCRO2D NetCDF file.
#   2. For each year's permit CSV:
...
#
# Inputs:
#   - Permit CSV dir: /home/jh94030/scripts/python/postdoc_project/rxfire/data/SE_permit_data_2010-2020/output_emis
#   - METCRO2D:       /scratch/jh94030/CMAQ-input/met/12US1/nc_classic/2017/mcip_v51_wrf_v411_noltng/01/METCRO2D_20170101.nc
#
# Outputs:
#   - /home/jh94030/scripts/python/postdoc_project/rxfire/data/gridded_CMAQ_12US1/Permit_CMAQ12US1_<YYYYMMDD>.nc
###############################################################################
...
PERMIT_CSV_DIR = r'/home/jh94030/scripts/python/postdoc_project/rxfire/data/SE_permit_data_2010-2020/output_emis'
OUTPUT_DIR     = os.path.join('/home/jh94030/scripts/python/postdoc_project/rxfire/data', 'gridded_CMAQ_12US1', 'Permit')
...
# Keys = CSV column names;  values = (long_name, original_unit)
SPECIES_DICT = {
    'CO':      'Carbon Monoxide',
    'CO2':     'Carbon Dioxide',
    'NH3':     'Ammonia',
...
def main():
    ...
    for year in YEARS:
        csv_path = os.path.join(
            PERMIT_CSV_DIR, f'SE_{year}_bluesky_rx_emis.csv'
        )
        if not os.path.isfile(csv_path):
            print(f'  CSV not found: {csv_path}  — skipping year {year}')
            continue

        print(f'======= Loading Permits {year} =======')
        df_all = pd.read_csv(csv_path)
        df_all['date'] = pd.to_datetime(df_all['date'])

        # --- Unit conversion on raw data ---------------------------------
        # area_acres: acres → m²
        df_all['area_acres'] = df_all['area_acres'] * ACRES_TO_M2
        ...
        for date in date_range:
            ...
            out_file = os.path.join(OUTPUT_DIR, f'Permit_CMAQ12US1_{ymd}.nc')
            ...
            if not found_df:
                ...
                arr_area = grid_species(df_day, 'area_acres', grid_info,
                                        resample='sum')
            ...
            # Global attributes
            ds_out.attrs['TITLE']   = ('Permit-based BlueSky rx fire emissions '
                                       'on CMAQ 12US1 grid')
            ds_out.attrs['HISTORY'] = (f'Created by Grid_Permits_to_CMAQ.py on '
                                       f'{pd.Timestamp.now()}')
            ...
            ds_out.attrs['FILTER']  = ('Prescribed fires from permit data '
                                       '(FL, GA, SC)')

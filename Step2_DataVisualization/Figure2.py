...
# -------------------- Config --------------------
ARIAL_PATH = "/home/jh94030/fonts/Arial.ttf"
dir_python_local = '/home/jh94030/scripts/python/postdoc_project/rxfire/data'
DATA_DIR = f"{dir_python_local}/SE_permit_data_2010-2020"   # expects dir_python_local in env
...
for st in STATE_ORDER:
    ...
    ax.plot(x, mean, label=STATE_LABELS[st], color=COLORS[st], linewidth=4)
    ax.fill_between(x, mean - std, mean + std, color=COLORS[st], alpha=0.3)

# Axis styling (unchanged)
ax.set_xlabel('Day of Week', fontsize=14, labelpad=20)
ax.set_xticks(np.arange(7))
ax.set_xticklabels(DOW_ABBR)
ax.set_ylabel('Fraction', fontsize=14, labelpad=20)
ax.tick_params(axis='x', labelsize=12)
ax.tick_params(axis='y', labelsize=12)
ax.grid(True, axis='y', linestyle=':', linewidth=0.5)

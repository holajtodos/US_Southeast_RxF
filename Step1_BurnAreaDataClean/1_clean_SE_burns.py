...
for selected_year in range(2010, 2021):
    ...
    for state_name in states:
        ...
        state_df.reset_index(drop=True, inplace=True)  # Reset index to 0...n-1
        state_df['OBJECTID'] = state_df.index + 1     # Assign a 1-based sequence
        ...
        if SHOW_PLOTS:
            ...
            if state.geom_type == "MultiPolygon":
                ...
                plt.plot(xs, ys, 'k')
            plt.scatter(state_df["LONGITUDE"], state_df["LATITUDE"], s=1)
            plt.show()
    ...
    for state_name in states:
        ...
        if os.path.exists(permit_file):
            ...
            if state_abbr == 'FL':
                ...
                check_conditions('NC', contains_agriculture_keyword=None, require_keywords_for_agri=['Other'])

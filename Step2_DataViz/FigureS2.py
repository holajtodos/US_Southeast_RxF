...
def plot_common(ax, gdf_SE, extent):
    ...
    ax.axis('off')
    ax.add_geometries(gdf_SE.geometry, crs=ccrs.PlateCarree(), facecolor='w',
...
def render_abs_map(out_png, grid_masked, cmaq_lon, cmaq_lat, gdf_SE, extent):
    ...
    print("Max % Burned:", np.nanmax(grid_masked.compressed()))
    print("Min % Burned:", np.nanmin(grid_masked.compressed()))

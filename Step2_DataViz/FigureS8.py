...
if SEASON == "all":
    ...
    im = ax.pcolormesh(cmaq_lon, cmaq_lat, mean_fire_pm25_masked, vmin=0, vmax=12,
                   cmap=cmap_new, transform=ccrs.PlateCarree(), zorder=2)
...
if SEASON == "all":
    ...
    bounds_pm = [0, 3, 6, 9, 12]   # keep same colorbar bounds
    xticks = np.linspace(0, 12, 5)               # custom x-axis ticks for gradient plot
    xtick_labels = [f"{v:.2g}" for v in xticks]

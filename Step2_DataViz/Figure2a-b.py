...
selected_GA_counties_gdf = gpd.read_file('nonattainment_GA_counties_obs.shp')
selected_FL_counties_gdf = gpd.read_file('nonattainment_FL_counties_obs.shp')
...
def mask_from_gdf(lon_grid, lat_grid, gdf):
    ...
    # Loop—robust across Shapely versions and avoids heavy GeoSeries construction
...
def multi_year_annual_mean_pm25(years):
    """
    For each year: mean over days (TSTEP) of daily Rx PM2.5 -> annual mean array.
    Then average these annual means across years.
    """
    annual_means = []
    for yr in years:
        print(f"[PM2.5] Processing year: {yr}")
        ds_all, ds_rmv = open_cmaq(yr)
        fp = fire_pm25_year(ds_all, ds_rmv)  # (TSTEP, Y, X)
        annual_means.append(fp.mean(dim='TSTEP').values)  # (Y, X)
...
def multi_year_avg_smoke_days(years, threshold):
    """
    For each year: count of days with Rx PM2.5 > threshold at each grid cell.
    Then average the counts across years to get 'annual average high Rx fire smoke days'.
    """
    per_year_counts = []
    for yr in years:
        print(f"[SmokeDays] Processing year: {yr}")
        ds_all, ds_rmv = open_cmaq(yr)
        fp = fire_pm25_year(ds_all, ds_rmv)                # (TSTEP, Y, X)
        counts = (fp > threshold).sum(dim='TSTEP').values  # (Y, X)
...
ax.add_geometries(selected_GA_counties_gdf.geometry, crs=ccrs.PlateCarree(), facecolor='none',
                  edgecolor='#8C0909', linewidth=1, zorder=3)
ax.add_geometries(selected_FL_counties_gdf.geometry, crs=ccrs.PlateCarree(), facecolor='none',
                  edgecolor='#8C0909', linewidth=1, zorder=3)

im = ax.pcolormesh(cmaq_lon, cmaq_lat, mean_fire_pm25_masked, vmin=0, vmax=4.5,
...
cbar.set_label('Annual average Rx fire $\\mathrm{PM}_{2.5}$ ($\\mu g/m^3$)', fontsize=7, fontweight='bold')
for l in cbar.ax.xaxis.get_ticklabels():
    l.set_fontsize(7)

plt.savefig(os.path.join(dir_python_local, "pm25_fire_2017_2019.png"), bbox_inches='tight', dpi=600)
plt.close()
...
# Simple (no NaNs)
print(f"FL  min={min(data_fl)}, max={max(data_fl)}")
print(f"GA  min={min(data_ga)}, max={max(data_ga)}")
print(f"SC  min={min(data_sc)}, max={max(data_sc)}")
...
axarr[-1].set_xlabel("Annual average Rx fire $\\mathrm{PM}_{2.5}$ ($\\mu g/m^3$)", fontsize=15, fontweight='bold')

plt.subplots_adjust(left=0.1, right=0.98, top=0.95, bottom=0.15, hspace=0.1)
plt.savefig(os.path.join(dir_python_local, "joyplot_gradient_rx_fire_pm25.png"), dpi=600)
plt.close()
...
cbar.set_label('Annual average Rx fire smoke-affected days', fontsize=7, fontweight='bold')
for l in cbar.ax.xaxis.get_ticklabels():
    l.set_fontsize(7)

plt.savefig(os.path.join(dir_python_local, "pm25_fire_smoke_days_2017_2019.png"),
...
# Simple (no NaNs)
print(f"FL  min={min(data_fl_sd)}, max={max(data_fl_sd)}")
print(f"GA  min={min(data_ga_sd)}, max={max(data_ga_sd)}")
print(f"SC  min={min(data_sc_sd)}, max={max(data_sc_sd)}")
...
axarr[-1].tick_params(axis='x', bottom=True, direction='out')
axarr[-1].set_xlabel("Annual average Rx fire smoke-affected days", fontsize=15, fontweight='bold')

plt.subplots_adjust(left=0.1, right=0.98, top=0.95, bottom=0.15, hspace=0.1)
plt.savefig(os.path.join(dir_python_local, "joyplot_gradient_rx_fire_smoke_days.png"), dpi=600)
plt.close()